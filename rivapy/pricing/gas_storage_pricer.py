...
def price_lsmc(storage: GasStorageSpecification, 
                pricing_parameters: PricingParameter,
                prices: np.ndarray):
    """_summary_

    Args:
        storage (GasStorageSpecification): _description_
        pricing_parameters (PricingParameter): _description_
        ...
        for t in range(len(self.timegrid),1,-1):
            ...
            for vol_tplus1 in range(len(v)): #for all volumes by themselves
                ...
                for vol_t in range(len(v)):
                    ...
                    dec_func[vol_t,:] = _payoff_func(S[t-1,:],dv) + cv_pred
                ...
                # - Calculate the accumulated future cash flows Y^b
                acc_cashflows[t-1,vol_tplus1,:] = _payoff_func(S[t-1,:], dv_max) + acc_cashflows[t, vol_tplus1, :] #no disc. factor
